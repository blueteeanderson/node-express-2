...
describe("GET /users", function() {
  ...
  // TESTS BUG 4
  test("should only display basic information", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(Object.keys(response.body.users[0]).sort()).toEqual(['username', 'first_name', 'last_name'].sort());
  });
...
describe("GET /users/[username]", function() {
 ...
 // TESTS BUG 1
  test("should deny access to u1 for u2 information", async function() {
    const response = await request(app)
      .get("/users/u2")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(401);
...
describe("DELETE /users/[username]", function() {
  ...
  test("should allow if admin and query", async function() {
    // TESTS BUG 5 alough this works it shouldn't be put in the query
    // depending on size of the token it will get cut of
    const response = await request(app)
      .delete("/users/u1")
